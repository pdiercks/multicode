...
def adaptive_edge_rrf(
    ...
    **sampling_options,
    ...
    for i in range(dof_layout.num_entities[1]):
        ...
        edge_mesh = tp.subproblem.edges[edge]
        ...
        range_bc = dolfinx.fem.dirichletbc(np.array((0, ) * (facet_dim + 1), dtype=ScalarType), _dofs_)
        edge_boundary_dofs[edge] = range_bc.dof_indices()[0]

        # ### range product
        inner_product = InnerProduct(edge_space, range_product, bcs=(range_bc, ))
        range_product = FenicsxMatrixOperator(inner_product.assemble_matrix(), edge_space, edge_space)
        range_products[edge] = range_product
        ...
        shape_funcs = line.interpolate(edge_space, component)
        N = range_spaces[edge].from_numpy(shape_funcs)
        coarse_basis[edge] = N

        # ### edge test sets
        dofs = tp.subproblem.V_to_L[edge]
        test_sets[edge] = range_spaces[edge].from_numpy(M.dofs(dofs))
    ...
    # NOTE tp.source is the full space, while the source product is of lower dimension
    num_source_dofs = len(tp._bc_dofs_gamma_out)
    testfail = np.array([failure_tolerance / min(num_source_dofs, Srange.dim) for Srange in range_spaces.values()])
    testlimit = (
        np.sqrt(2.0 * lambda_min) * erfinv(testfail ** (1.0 / num_testvecs)) * error_tol
    )
    ...
    while np.any(maxnorm > testlimit):
        basis_length = [len(rb) for rb in pod_bases.values()]
        ...
        for edge in target_edges:
            ...
            # extend pod basis
            extend_basis(U_fine, rb, product=None, method="pod")
