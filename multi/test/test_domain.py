...
def test():
    ...
    other = Domain(mesh, id_=1, subdomains=None, edges=False, translate=df.Point((2.1, 0.4)))
    assert other.subdomains is None
    assert np.isclose(other.ymax, 1.4)
    assert np.isclose(other.xmax, 3.1)

    another = Domain("data/rvedomain.xdmf", id_=2, subdomains=True, edges=True, translate=None)
